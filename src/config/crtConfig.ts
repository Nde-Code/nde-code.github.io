...
export function applyCRTConfig(config: Partial<CRTConfig>) {
  ...
  if (scanlineOpacity === undefined) {
    const savedIntensity =
      (localStorage.getItem('tui_crt_intensity') as CRTIntensityLevel) || 'medium';
    scanlineOpacity =
      CRT_PRESETS[savedIntensity]?.scanlineOpacity ?? DEFAULT_CRT_CONFIG.scanlineOpacity;
    vignetteOpacity =
      CRT_PRESETS[savedIntensity]?.vignetteOpacity ?? DEFAULT_CRT_CONFIG.vignetteOpacity;
  }
